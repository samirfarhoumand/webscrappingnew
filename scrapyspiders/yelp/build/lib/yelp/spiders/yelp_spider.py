...
class YelpSpider(Spider):
    ...
    start_urls = ["https://www.yelp.com/search?find_desc=gyms&find_loc=Los%20Angeles%2C%20CA&sortby=review_count&start=0"]
    ...
    # List comprehension to construct all the urls
        result_urls = [f"https://www.yelp.com/search?find_desc=gyms&find_loc=Los%20Angeles%2C%20CA&sortby=review_count&start={i*10}" for i in range(0, 100)]
    ...
    def parse_results_page(self, response):
        ...
        for url in gym_urls:
            yield Request(url = url, callback=self.parse_gym_page, meta = {
                'current_url':url
                })

    def parse_gym_page(self, response):
        ...
        num_pages = math.ceil(num_review/20)
        gym_review_urls = [f"{current_url}"] + [f"{current_url}?start={i*20}" for i in range(1,num_pages)]

        print("="*20)
        print(gym_review_urls)
        print("="*20)

        #Yield the requests to the gym reviews pages,
        # using parse_gym_reviews_page function to parse the response

        for url in gym_review_urls:
